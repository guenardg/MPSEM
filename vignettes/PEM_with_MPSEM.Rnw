...
and predicting trait values using phylogeny as an explanatory factor
...
values are being predicted (hereafter referred to as the ``target
...
and ``vertex'' over ``root'', ``node'' or ``tip''; safe in
cases where I want to be specific about what a vertex represents.
...
the weighted and centred influence matrix and 4) use these eigenvectors
...
from R package {\tt caper}. These data from \citet{Purvis1995} are
loaded into your {\tt R} workspace as follows:
...
The last code line is just a last check to guarantee that all species
...
As previously announced, I use the vocabulary of the graph theory
when describing PEM: a tree is a (directed) graph, a branch is an
edge, and the root, nodes, and tips are vertices. PEM allows one to
specify a model of trait evolution along the edges of the tree. That
model is given as a function having edge lengths as its first argument,
...
parts of the phylogeny as well as arbitrary weighting functions, the
...
event, trait either change abruptly (directional selection) at the
...
and called {\tt \$species} stores whether a given vertex represents
a species (i.e., it is a tip). In addition to edge and vertex information,
...
that does step-wise variable addition in multiple regression analysis
...
\end{figure}
 From the present cross-validation, we found that the ($\log$) neonatal
body mass can be predicted with a cross-validated $R^{2}$ of \Sexpr{round(1-(sum((perissodactyla.data[,"predictions"]-perissodactyla.data[,"log.neonatal.wt"])**2)/nrow(perissodactyla.data))/var(perissodactyla.data[,"log.neonatal.wt"]),2)}
